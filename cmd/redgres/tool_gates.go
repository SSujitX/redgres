...
func startToolGates(cfg config.Config, store *toolgate.Memory, log *slog.Logger) {
	if store == nil {
		return
	}
	startOne := func(tool, listen, upstream string) {
		if listen == "" || upstream == "" {
			return
		}
		gate, err := toolgate.NewGate(tool, upstream, store, cfg.CookieSecure, cfg.BaseURL)
		if err != nil {
	...
	startOne(toolgate.ToolPgAdmin, cfg.ToolGatePgAdminListen, cfg.ToolGatePgAdminUpstream)
