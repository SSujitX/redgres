...
Redgres has a compiling Wave 0 foundation, owner auth, a browser login/shell, read-only PostgreSQL inventory (API + Databases page), a table-list API plus inspector list, a bounded row-browse API, inspector row paging/search, paginated audit history, Overview component status (Redis Ping on `GET /api/v1/status` plus metrics on `GET /api/v1/redis/status`), and authenticated bounded global search (PostgreSQL names + client navigation; Redis ACL hits are not implemented). Redis Ping and INFO/DBSIZE/Ping-RTT metrics exist when a URL file is configured; the adapter is not a full ACL admin. No vault decrypt, mutations, Redis ACL administration, or installer is implemented. Target documentation is not evidence that those features exist.
