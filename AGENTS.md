...
Redgres has a compiling Wave 0 foundation, owner auth, a browser login/shell, read-only PostgreSQL inventory (API + Databases page), a table-list API plus inspector list, a bounded row-browse API, inspector row paging/search, paginated audit history, Overview component status (Redis Ping on `GET /api/v1/status` plus metrics on `GET /api/v1/redis/status`), authenticated bounded global search (PostgreSQL names + client navigation; Redis ACL hits are not implemented), and Redis ACL list/inspect (`GET /api/v1/redis/users` and username detail + ACL users page). Redis Ping, INFO/DBSIZE/Ping-RTT metrics, and ACL LIST parse exist when a URL file is configured; the adapter is not a full ACL admin (no create/rotate/delete). No vault decrypt, mutations, or installer is implemented. Target documentation is not evidence that those features exist.
