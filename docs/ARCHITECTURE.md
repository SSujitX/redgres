...
- operation records and resource locks (`operations`, `operation_locks`; ADR-010). Columns never store secrets. This freeze persists only `postgres.database.duplicate`. GET `/api/v1/operations/{id}` is session + `platform.read`. Duplicate POST is **202 after InsertQueued** (no credential). Worker `RunQueuedDuplicates` writes the vault row; SQLite never stores the password.
