...
### Expert tools + pgAdmin master password (2026-08-31)

PLAT-001 / OPS-001 / ADR-014 Partial: System **Expert tools** nav (Open + Reveal). Install generates `pgadmin.pass` + `pgadmin.master` + official `MASTER_PASSWORD_HOOK`; finish box TTY-only login/master (honest “no TTY” / Reveal copy); Reveal may return `master_password`. `[3/8] Redis` = Insight only; `[4/8] PostgreSQL` = pgAdmin. Upgrade rewrites expert compose before systemd restart (env key loaded), fail-closed when compose exists; warns if old volume may need recreate. App-only `install.sh` / `install-dev.sh` restore `5050:redgres` after secrets `chown -R`. Commands: `go test ./internal/config/ ./internal/httpapi/ -count=1 -run 'TestLoadToolGateLoopback|TestPgAdminReveal'` (ok); `cd web && npx vitest --run src/nav.test.ts src/features/system/ExpertToolsSection.test.tsx` (12 passed); Git Bash `bash deploy/tests/run.sh` (203 passed). Not done: live finish-box on Ubuntu, existing pgAdmin volume recreate, §6. Do not mark Complete.
