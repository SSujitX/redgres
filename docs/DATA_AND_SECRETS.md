...
If systemd runs as `redgres`, root-only source files cannot be read directly after privilege drop. The fresh installer therefore keeps the generated legacy-vault source and its exact managed-key marker `root:root 0600`, then uses `LoadCredential=` to expose an ephemeral read-only copy in the unit credential directory. Only the root-owned marker authorizes privileged unit regeneration to select the canonical source; app-writable `redgres.env` cannot suppress or replace it. Ubuntu systemd presents the private copy as `root:root 0440`; production accepts only that exact filename, ownership, mode, and active `/run/credentials/...` path. The unit's `ExecStart` forces production mode and sets the managed credential path; when no trusted marker exists it explicitly clears the vault variable. `EnvironmentFile` values cannot override either property. During an upgrade, a legacy source is adopted only when root stages the exact bytes as `root:root 0600` beneath trusted ancestors and writes that path to the one-time `root:root 0600` `/etc/redgres/secrets/legacy-vault-secret.adopt` manifest. The privileged installer descriptor-pins the manifest, staged source, and configured owner-only legacy source, requires bounded non-symlink files beneath non-writable root/owner ancestors, compares exact bytes, copies to the canonical source, creates the marker, and consumes the manifest; missing authorization, duplicates, mismatch, unsafe metadata, collisions, and invalid markers fail before service mutation. Rollback to a pre-1.1.9 reader keeps the root source authoritative and copies the systemd credential into a service-private `0600` runtime file before starting the old binary. Ordinary group-readable `0640` sources remain rejected.
