...
`database.CaptureSQLiteSnapshot` is the bounded producer for the preferred online path. It accepts an already-open SQLite handle and an existing private staging root, creates an identity-pinned operation-private directory and exclusive target, verifies the pinned driver's pathname-reopened destination before its first copy step, and copies in cancellation-aware batches. It independently requires the standalone copy's `PRAGMA integrity_check` result to be exactly `ok` between two matching size/SHA-256 passes. The returned path is the retained file inside that generated operation directory.

`database.VerifySQLiteSnapshotRestore` performs isolated verification of that exact producer artifact and accepts snapshots no larger than 512 MiB. On Linux it copies and hashes the pinned bytes into a write/grow/shrink/seal-protected `memfd`, confirms the complete seal set, re-hashes the frozen descriptor, and binds the driver source through `/proc/self/fd`; the lexical non-Linux source is development-only. The verifier checks retained identity and metadata, opens the sealed source read-only, restores in bounded steps into one fresh connection-owned in-memory database, requires exact integrity success and zero foreign-key violations, verifies a bounded complete embedded migration set with exact checksums and no missing/newer version, and returns non-secret counts for owners, sessions, login attempts, audit events, operations, and operation locks. The destination is discarded and matching original source size/SHA-256 is required afterward. This is not whole-set or production restore evidence: neither function publishes `current.json`, applies retention, performs off-host transport, or coordinates PostgreSQL/Redis/config restore, and the live installer backup command does not call them yet.
...
Live dump, off-host copy, restore, and `deploy/backup.sh` are installer-recovery. This section freezes the filesystem manifest types used by `backup.EvaluateDropGate` ([ADR-011](decisions/ADR-011-drop-backup-gate.md)). There is no SQLite backup table. The catalog jail is `REDGRES_BACKUP_CATALOG`. The only manifest file is jail-root **`current.json`**; artifact files referenced by that manifest also live below the jail. Clients never supply a path.
...
Off-host: `off_host.completed=true` and `completed_at` ≤ `copied_at` ≤ evaluation `Now` for this set. Off-host age is not a second 24h window. No transport credentials on the manifest.

Restore evidence: `restore.isolated=true`, `restore.outcome=succeeded`, `restore.backup_set_id` equal, and `completed_at` ≤ `restore.completed_at` ≤ evaluation `Now`; restore evidence must also be ≤30d old. Evaluation `Now` must be non-zero (fail closed).

`completed_at` must not be in the future and must be ≤24h old at evaluation time. `ParseManifest` reads at most 8 MiB, accepts at most 1024 artifacts before typed allocation, requires valid UTF-8 and well-formed JSON surrogate pairs, requires exact case-sensitive root/nested schema keys, rejects duplicate keys, and bounds JSON depth/tokens plus artifact kind/name/path, system identifier, restore evidence, and Redgres identity text. Fail closed on missing fields, extra JSON, case aliases, traversal (`..`, absolute, NUL, `?`, `#`, `%`), oversized input, or identity mismatch. Truncate and row-delete stay ungated. Optional empty `redgres.version` / `redgres.compatibility_policy_revision` do not fail DROP evaluation. Redis/SQLite artifacts are optional for this predicate. `EvaluateDropGate` remains a pure manifest predicate and does not read artifact bytes. After that predicate allows HTTP DROP, `backup.VerifyPostgresDatabaseArtifact` requires exactly one matching `postgres.database` artifact, opens the existing regular file without creating paths or following symlinks, streams its bytes with request cancellation, rechecks file identity, and verifies exact size plus lowercase SHA-256 before terminate/`DROP DATABASE`. Missing, replaced, non-regular, wrong-size, or wrong-hash artifacts fail closed as a secret-safe `503`.
