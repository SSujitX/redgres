...
Until a non-eval credential path exists, installer existing-mode `--dry-run` inventories PATH host binaries with `postgres --version`, `redis-server --version`, and `pgbouncer --version` (no `-D`/`PGDATA`, no Redis conf, no PgBouncer ini; not `psql -V`, not `redis-cli`, not SQL `SHOW` / Redis `INFO` / PgBouncer `SHOW VERSION`). Runtime Redgres detection remains `SHOW server_version_num`, `INFO server` (`redis_version`), and `SHOW VERSION` as specified above. A host `--version` string is not proof that a cluster is running and does not widen this matrix.
