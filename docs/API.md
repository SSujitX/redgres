...
**Implemented now (this freeze SHA):** `POST /api/v1/postgres/databases/{db}/duplicate` remains **201 in-request** with a credential. It does not allocate an operation ID.

**Frozen 202 fixture (writers implement; do not treat as implemented on this SHA):** after enqueue, that POST returns always `202` `Cache-Control: no-store` with no credential and never waits for `CREATE DATABASE … TEMPLATE`. Ignore extra `credential` / `password` / URL fields if present.
...
7. Backup gate, after AUTH-006 and before terminate/`DROP DATABASE`: empty/unset `cfg.BackupCatalogDir` → `503` `dependency_unavailable`, copy **Backup catalog is not configured.** (never echo the path; no DROP SQL). Load jail-root `current.json` only (`backup.LoadCurrent` / `ParseManifest`; no client path). Missing/unreadable catalog → same 503. Live identity is `Catalog.SystemIdentifier` (`SELECT system_identifier FROM pg_control_system()`, bigint decimal string). `EvaluateDropGate` with `time.Now().UTC()` and path `{db}`. Deny → `403` `forbidden` with the evaluator reason exactly. Parse/identity/Evaluate failures that are not those deny reasons stay `503` without echoing paths or `err.Error()`.
8. TryLock. Then `Service.Drop`.
9. Success audit then **200**. Audit-fail after `DROP DATABASE` → `503` `dependency_unavailable` (database stays dropped; same fail-closed family as truncate / Redis DELETE).
...
**Audit:** success `postgres.database.duplicate` with metadata `database` (new name), `owner` (new role), and `source` (path `{db}`). Optional `operation_id` (32 lowercase hex) is allowed when a 202 worker records the ledger id. Unknown keys remain rejected.
