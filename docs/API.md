...
**Implemented now:** `POST /api/v1/postgres/databases/{db}/duplicate` always returns **202** after `InsertQueued` (`Cache-Control: no-store`) with no credential and never waits for `CREATE DATABASE … TEMPLATE`. Ignore extra `credential` / `password` / URL fields if present. Worker `postgresadmin.RunQueuedDuplicates` / `NewDuplicateProbe` / `NewDuplicateCompensator` exist; parent wires Open → Reconcile(live Probe, Compensator) → 1s `ListQueued` poller in `cmd/redgres/main.go` after this SHA.
