...
1. **Cloudflare path:** choose API token (paste once) or OAuth (only after the live console hostname). API token apply `{zone, origin_ip, hostnames}` creates the tunnel, proxied CNAME, grey-cloud db/rs **A or AAAA**, and deny-by-default Access apps; durably saves the deployment, then queues Let's Encrypt for db/rs. TLS reuses a valid matching canonical or suffixed Certbot lineage before ACME, persists host-bound cooldowns, transactionally applies PostgreSQL/PgBouncer certificates, and ignores unrelated renewals. `rs` remains `certificate_prepared` until Redis TLS is actually applied. Disconnect is retryable/acknowledged, removes the root renewal credential, and preserves lineage/cooldown. A leftover tunnel named `redgres-{console}` with status `inactive` or `down` and no connections is replaced; a healthy/connected tunnel conflicts. Access allow emails → operator confirms console reachable → bootstrap closes. The wizard does not ask for OAuth after an API-token apply.
