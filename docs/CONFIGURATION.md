...
| `REDGRES_TLS_CERT_DIR` | No | `/etc/ssl/redgres` | Destination for Let's Encrypt fullchain/privkey copies used by PostgreSQL and PgBouncer. Must be traversable by `postgres`/`ssl-cert` (not `/etc/redgres`, which is `0750` `root:redgres`). |
