...
| `REDGRES_CERTBOT_BIN` | No | `certbot` | Development/direct-issuer override only. The production root helper uses the fixed executable from its sanitized system path. |
| `REDGRES_CERTBOT_DNS_TOKEN_FILE` | No | `/var/lib/redgres/secrets/certbot-dns.ini` | certbot-dns-cloudflare credentials file (never returned). This path is installer-owned and fixed in production so the unprivileged app and root helper cannot diverge. |
| `REDGRES_TLS_ISSUE_REQUEST_FILE` | No | `/var/lib/redgres/tls-issue.request` | Hostnames-only request file for `redgres-tls-issue.path`. This path is installer-owned and fixed in production. |
| `REDGRES_TLS_ISSUE_RESULT_FILE` | No | `/var/lib/redgres-tls/issue.result` | Atomic helper result/cooldown in the root-owned systemd state directory: `issued` + covered hostnames, or `failed` + an allow-listed reason and optional RFC 3339 retry time. The app can read but cannot replace/delete it. Sibling root files are `active.request`, `cleanup.result`, and the `0600` renewal credential snapshot. |
| `REDGRES_TLS_CERT_DIR` | No | `/etc/ssl/redgres` | Root-helper staging directory; not an application production override. A root-owned `/etc/redgres/tls-targets` manifest limits service updates to the cluster and PgBouncer instance explicitly created by the full-stack installer. |
...
| `REDGRES_LEGACY_VAULT_SECRET_FILE` | Implemented | Optional path to the exact legacy KDF secret (`SESSION_SECRET` bytes). File path only. Not part of `PostgresConfigured` / `postgresAnySet`. Fresh install generates the root-owned source once at `/etc/redgres/secrets/legacy-vault-secret`, preserves it on rerun, and records root-only managed-key metadata that the service cannot forge. A managed source always reaches the app through a read-only systemd credential, regardless of app-writable `redgres.env`; the source path is never written there. The installed unit forces production mode and explicitly clears any unmarked vault env value, so unsupported/manual adoption fails closed instead of trusting service-writable selection. A future privileged adoption workflow must create equivalent root-owned authority without replacing legacy key material. When set in a trusted runtime and PostgreSQL is configured, `postgresadmin.Open` applies fail-closed regular-file/no-symlink checks (`TrimRight` `\r\n`; empty rejected). Production accepts owner-only files, plus only systemd's exact root-owned `0440` `legacy-vault-secret` under the active `/run/credentials/...` directory; other group/world access is rejected. Errors name this env var and never echo contents. Open stores the derived Fernet key on the service and wipes the raw secret. Unset → Open succeeds; POST reveal and POST create return `503`. Does not mark PostgreSQL configured. PG-003 create does not add `REDGRES_POSTGRES_CONNECTION_LIMIT` (role limit is constant 20). |
...
| `REDGRES_PGADMIN_MASTER_PASSWORD_FILE` | Implemented | Optional path to the generated pgAdmin master-password / `MASTER_PASSWORD_HOOK` key. Production path must be under `/var/lib/redgres`. Reveal includes `master_password` only when this regular file is present and readable; missing file omits the key and still returns login. Never returned on GET `/session`. |
