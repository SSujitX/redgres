...
| `REDGRES_BOOTSTRAP_ALLOW_FROM` | No | *(empty)* | Optional single operator IP for the installer UFW rule (`ufw allow from IP to any port 8989`). When unset, the installer uses the SSH client address from `SSH_CONNECTION` / `SSH_CLIENT`. Loopback, unspecified, and CIDR values are rejected. Never opens `8989/tcp` to the world. |
...
| `REDGRES_CERTBOT_DNS_TOKEN_FILE` | No | `/var/lib/redgres/secrets/certbot-dns.ini` | certbot-dns-cloudflare credentials file (never returned). Live install/upgrade writes this path. |
