...
6. Truncate.
7. Drop database/conditional role cleanup. PG-011 Partial is in-request **200** (terminate excluding current backend, then `DROP DATABASE`, optional `DROP ROLE` + vault DELETE) after the HTTP backup gate (`LoadCurrent` + `EvaluateDropGate`). Unset catalog is `503`. Live dump/copy/restore remains the OPS-004 Complete blocker.
