...
- Duplicate finishes ownership transfer with blanket `GRANT ALL` / `ALTER DEFAULT PRIVILEGES` in `public` and does not remove old-owner memberships added for transfer. Redgres must preserve ACLs on skipped protected/superuser objects, direct extension members identified on their own catalog row by `pg_depend.deptype = 'e'`, and `SECURITY DEFINER` routines, and keep any necessary transfer membership transaction-scoped. Subsidiary/internal extension objects without direct extension membership remain a PostgreSQL 17/18 live-test question, not a local implementation claim.
