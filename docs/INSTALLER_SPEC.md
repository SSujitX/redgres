...
9. **Application release** — verify artifact checksum, install immutable release, migrate SQLite, configure systemd credentials/unit. **This Partial (OPS-005):** `update --non-interactive --dry-run --release PATH` and `rollback --non-interactive --dry-run --to VERSION` only; PATH must pass the trusted regular-file policy and is read only through a pinned descriptor for adjacent `SHA256SUMS` verification, never sourced, evaluated, extracted, or printed. VERSION is a path-safe token (no `/`, `.` or `..` as a path component, no NUL) and is never printed. Prints an explicit skip matrix (`result=partial`). Update does not verify signature/provenance, validate archive members, write `/opt/redgres/releases`, switch `current`, migrate SQLite, write systemd units/credentials, probe GET `/api/v1/healthz`, or install PostgreSQL packages. Rollback does not switch the symlink, check SQLite schema compatibility, restore config, restart systemd, probe healthz, or reverse PostgreSQL/Redis/vault/credentials/DNS/schema. Missing `--non-interactive`, missing/untrusted release or checksum evidence, checksum mismatch/ambiguity, missing/unsafe `--to`, or unknown flags (including `--config`/`--mode` on update) exit 1. Live update/rollback without `--dry-run` exits 2. Does not call tar, ln, curl, gpg, pg_dump, pg_restore, or redis-cli. This Partial is not Complete.
