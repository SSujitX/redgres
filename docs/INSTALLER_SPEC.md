...
**This Partial (OPS-007):** `postgres-plan --config PATH --extension-plan PATH` is a read-only extension-plan validator. `--config` and `--extension-plan` must be existing regular files; `--config` is never sourced, evaluated, or printed. It validates the non-secret plan JSON: `policy` is `preserve` or `apply-selected`; every `selections` entry has a capability ID from the release-owned registry (the initial list in [POSTGRESQL_PROVISIONING.md](POSTGRESQL_PROVISIONING.md) section 5), a non-empty explicit `databases` array of PostgreSQL-identifier-safe names (never protected names like `postgres`, `template0`, `template1`, `database_console_vault`, never an empty list that implies "all databases"), and an optional `scheduler` that is valid only for `pg_partman` (`pg_partman_bgw`, `pg_cron`, or `external`). The whole plan must be valid JSON with no unknown keys; malformed JSON, unknown capabilities, invalid or protected database names, scheduler misuse, and two schedulers fail closed (exit 1). It prints a plan preview and a skip matrix (`result=partial`): package resolution (no release manifest), live inventory, backup verification, preload merge, restart approval, extension DDL, and smoke verification are all skipped. It never mutates, never sources `--config`, never touches `template1`, and never resolves packages/preload/restart. This Partial is not Complete.
**This Partial (OPS-004):** `backup --non-interactive --dry-run --config PATH` prints a skip matrix (`result=partial`). `--config` must be an existing regular file and is never sourced, evaluated, or printed. It does not invoke `pg_dump`/`pg_dumpall`/`pg_restore`, Redis `BGSAVE`/`LASTSAVE`, SQLite online backup, checksums, pruning, or off-host copy — all are skipped (`installer-recovery`), and no backup manifest is written. Live backup without `--dry-run` exits 2. This Partial is not Complete.
