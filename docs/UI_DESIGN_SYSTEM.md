...
- Product identity uses the full Redgres wordmark in the application shell and login surface. The compact mark is favicon-only. Both assets are transparent PNGs under the allow-listed `/assets/` namespace; the current light interface uses the dark-letter wordmark, while the light-letter variant is reserved for an explicitly activated and tested dark theme.
...
- Allow-list command catalogs (Edit permissions Custom and Create ACL user Custom) use `.command-checklist`: one labeled checkbox per row, `min-height: 44px`, `min-width: 0`, `--space-2` label gap, and `--line` / `--radius-surface` fieldset chrome. Do not wrap commands as inline labels. The Permission presets page is a read-only named-command catalog: page `h1`, named presets `h2`, Queue/worker kinds `h3`, and `.rule-list`, never `.command-checklist`.
