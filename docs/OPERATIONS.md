...
- db/rs certificate flow: apply saves deployment state before queueing. The root helper snapshots the DNS token into `/var/lib/redgres-tls`, reuses an exact valid lineage before ACME, enforces host-bound cooldown, validates cert↔key, and transactionally updates only `/etc/redgres/tls-targets`. Reload failure or served-leaf mismatch restores every cert/config copy. `/etc/redgres/tls-lineage` gates renewals; unrelated certificates are ignored. Disconnect waits for root cleanup acknowledgement, deletes the renewal token snapshot, and preserves lineage/cooldown for reconnect. Redis public TLS enablement is not part of this Partial (`rs` is `certificate_prepared`; loopback `6380` stays plaintext).
- Domain & Network shows a secret-safe **Recent activity** log. Apply/TLS/disconnect also write durable `started` then terminal `success`/`failure` audit events before/after side effects. Server logs: `journalctl -u redgres.service --no-pager` for the app, `journalctl -u redgres-tls-issue.service --no-pager` for the privileged helper, and `/var/log/letsencrypt/letsencrypt.log` for Certbot. Redact before sharing; raw diagnostics are never returned to the browser.
