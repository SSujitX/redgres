#!/usr/bin/env bash
# Redgres development installer: compile master on this machine.
# GitHub publishes versioned releases only (install.sh / upgrade.sh).
#
#   sudo REDGRES_DEV_BUILD_LOCAL=1 ./install-dev.sh
#
set -euo pipefail
...
redgres_public_open_legacy_vault_source_for_comparison() {
  ...
  REDGRES_PUBLIC_LEGACY_COMPARE_FD="${fd}"
  REDGRES_PUBLIC_LEGACY_COMPARE_PID="${shell_pid}"
}
...
redgres_public_adopt_legacy_vault_secret() {
  ...
  local configured configured_source candidate manifest_fd manifest_path fd fd_path compare_fd compare_path tmp marker_tmp
  ...
  if [[ -n "${configured}" && "${configured_source}" != "${candidate}" ]]; then
    if ! redgres_public_open_legacy_vault_source_for_comparison "${configured_source}"; then
      exec {fd}<&-
      return 1
    fi
    compare_fd="${REDGRES_PUBLIC_LEGACY_COMPARE_FD}"
    compare_path="/proc/${REDGRES_PUBLIC_LEGACY_COMPARE_PID}/fd/${compare_fd}"
...
redgres_write_app_unit() {
  ...
  redgres_public_adopt_legacy_vault_secret || die 'legacy vault migration requires root authorization: stage the exact existing bytes as root:root 0600 under trusted ancestors, point legacy-vault-secret.adopt at that copy, and retry (docs/CONFIGURATION.md)'
...
wait_for_healthz() {
  redgres_wait_for_healthz "${HEALTHZ}" 20 || die "API check failed after install (check journalctl -u redgres)"
}

print_summary() {
  local version="$1" mode="${2:-installed}"
  redgres_print_install_summary "${version}" dev "${mode}"
}
...
EOF
  ...
  chown root:redgres "${ETC_ROOT}/redgres.env" 2>/dev/null || true
}
...
install_build_local() {
  ...
  redgres_public_adopt_legacy_vault_secret || die 'legacy vault migration requires root authorization before changing the current release (docs/CONFIGURATION.md)'
...
log "  ${DIM}Compile master on this machine (Go + Node). Versioned GitHub releases use install.sh.${NC}"
log ""

if [[ "${REDGRES_DEV_BUILD_LOCAL:-}" == "1" ]]; then
  install_build_local
else
  die "GitHub does not publish a rolling dev snapshot. Install a versioned release with install.sh, or compile here: sudo REDGRES_DEV_BUILD_LOCAL=1 $0"
fi
