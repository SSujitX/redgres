...
install_from_dev_release() {
  ...
  ASSET="${TARBALL_URL##*/}"
  redgres_assert_github_download_url "${TARBALL_URL}" "${ASSET}" "${TAG}" || die "dev tarball URL is not the expected GitHub asset"
  redgres_assert_github_download_url "${SUMS_URL}" "SHA256SUMS" "${TAG}" || die "dev SHA256SUMS URL is not the expected GitHub asset"
